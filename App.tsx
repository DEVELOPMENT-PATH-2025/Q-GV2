...
function AppContent() {
  ...
  useEffect(() => {
    if (firebaseUser && role) {
      setView('app');
      // Set default view based on role when first entering the app
      if (role === UserRole.FACULTY) setCurrentDashView('dashboard');
      else if (role === UserRole.ADMIN) setCurrentDashView('review_queue');
      else if (role === UserRole.SUPER_ADMIN) setCurrentDashView('reports');
    } else if (!loading) {
